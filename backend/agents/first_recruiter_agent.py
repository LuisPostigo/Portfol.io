...
class MistralRecruiterAgent(AgentBase):
...
"""
    ...
    def _handle_message(self, ch, method, properties, body):
        try:
            ...
            print("🐞 [RecruiterAgent] Received message:", message)
            if not message:
                return
    ...
    def _handle_resume(self, message):
        ...
        print(f"📤 Response sent for applicant {applicant_id}")
        ...
        if job_id != "unknown":
            try:
                ...
                print(f"❌ Error saving to matches.db: {db_error}")
        ...
        print(f"📨 HiringManagerAgent notified for applicant {applicant_id}")

    def start(self):
        print(f"🤖 MistralRecruiterAgent (Local) is listening on queue: {self.queue_in}")
