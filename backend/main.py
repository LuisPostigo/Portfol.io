...
from backend.pre_processing.files_to_db import insert_file_if_missing, update_file_status, initialize_and_import_all
...
def delete_file(id: str = Query(...), path: str = Query(...)):
    try:
        ...
        if os.path.exists(path):
            os.remove(path)
            print(f"(X) Deleted file at {path}")
        else:
            print(f"(!) File {path} not found on disk")
        ...
        conn.commit()
        conn.close()
        print(f"(X) Deleted applicant {id} from applicants.db")
        ...
        if os.path.exists(matches_db_path):
            ...
            conn.commit()
            conn.close()
            print(f"(X) Deleted all matches for applicant {id} from matches.db")
        else:
            print(f"(!) matches.db not found")

        return {"message": "(!) File and related records deleted"}

    except Exception as e:
        print(f"(E) Failed to delete: {e}")
        return {"error": str(e)}
...
def get_match_details(applicant_id: str, job_id: str):
    """
    Returns the two agent evaluations plus, if present, the full debate
    ...
    if raw:
        try:
            ...
            for item in raw_list:
                ...
                else:  # unexpected non‑dict item
                    transcript_struct.append({"source": "system", "text": str(item)})
        except Exception:
            # not valid JSON → treat as a single plain‑text entry
    ...
    return {
        ...
        "debate_transcript":    transcript_struct,        # list or empty
        "debate_winner":        row["debate_winner"],     # may be None
    }
...
def get_match_details(applicant_id: str, job_id: str):
    ...
    if row["debate_raw"]:
        try:
            transcript = json.loads(row["debate_raw"])
        except Exception:
            # fallback to plain text
            transcript = [{"source": "system", "text": row["debate_raw"]}]

    return {
        "recruiter_agent": row["recruiter_agent"],
        "hiring_manager_agent": row["hiring_manager_agent"],
        "debate_transcript": transcript,        # <‑‑ new field
        "debate_winner": row["debate_winner"],  # <‑‑ new field
    }
...
def run_processing_pipeline(all_files_info):
    ...
    try:
        ...
        # (1) Launches recruiter agent
        subprocess.Popen([
            'start', 'cmd.exe', '/k',
            'conda activate portfol_io && python -m backend.agents.first_recruiter_agent'
        ], shell=True)

        subprocess.Popen([
            'start', 'cmd.exe', '/k',
            'conda activate portfol_io && python -m backend.agents.third_hiring_manager_agent'
        ], shell=True)
        ...
        print("✅ All files processed and system triggered accordingly.")
